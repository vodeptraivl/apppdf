function setAddImage(){
	...
	$('body').on('click','.importLocal,.imgPaintLayer', e => {
		if(canPaint){
			setPaint(false);
		}
		if(cantText){
	...
	$('body').on('mouseleave','.importLocal,.imgPaintLayer', e => {
		if(!onDragResize){
			imgOnEneter = false;
		}else{
			resize00 = true;
		}
		// if($('.importLocal[current="true"]') == null && !onDragResize) {
		// 	imgFocus = "";
		// }
	});


	 $('body').on('touchstart','.imgContainer',(e)=>{
	...
	$('body').on('touchend','.imgContainer',(e)=>{
		onImgTouchZoom = false;
	});

	$('body').on('touchmove','.imgContainer',(e)=>{
		// if(cantText){
			if(e.touches.length >= 2){
				let touchs = e.touches;
				let touch1 = {x:touchs[0].clientX,y:touchs[0].clientY};
...
function initImg(name,img,page){
	...
	historyMain.push({type:'img',page:(pageCurrent-1), data : imgOnBoard[idx] , idx:0 ,status:'init'});
...
function addHisImg(id,status){
	let item = imgOnBoard.find(x => {return x.id == id;});
	// item.status = status;
	let idx = (item.position.length-1);
	historyMain.push({type:'img',page:item.page, data : item,idx : idx,status:status});
	clearHistoryRedo();
	// let pos = (item.position[idx]);
	// $('#'+item.idDel).css({'width':pos.width,'height':pos.height,'transform': 'translate('+pos.x+'px, '+(pos.y)+'px)'});
}
